...
class SortByPriceDescendingUseCaseTest {

    private lateinit var useCase: SortByPriceDescendingUseCase
    ...
    @BeforeEach
    fun setup() {
        useCase = SortByPriceDescendingUseCase()
    }

    @Test
    fun `invoke returns products sorted by price in descending order`() {
        // Given
        val products = listOf(
            product.copy(id = 1, price = 10.0),
            product.copy(id = 2, price = 15.0),
            product.copy(id = 3, price = 5.0)
        )
